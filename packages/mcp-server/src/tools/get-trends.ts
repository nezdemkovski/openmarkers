    ...
    "get_trends",
    {
      ...
      inputSchema: z.object({
        ...
        biomarker_id: z
          .string()
          .optional()
          .describe("Optional: filter to a specific biomarker ID"),
        category_id: z
          .string()
          .optional()
          .describe("Optional: filter to a specific category ID"),
      }),
    },
    async ({ profile_id, biomarker_id, category_id }) => {
      const trends = await getTrendsForProfile(profile_id, authUserId, {
        biomarker_id,
        category_id,
      });
      ...
      if (trends.length === 0)
        return mcpText(
          "No trends available (need at least 2 results per biomarker)",
        );
