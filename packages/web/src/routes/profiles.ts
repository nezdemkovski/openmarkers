...
export async function handleListProfiles(auth: { userId: string }): Promise<Response> {
  return json(await listProfiles(auth.userId));
}

export async function handleGetProfile(auth: { userId: string }, id: number): Promise<Response> {
  const data = await getProfileData(id, auth.userId);
  if (!data) return error("Profile not found", 404);
  return json(data);
}

export async function handleCreateProfile(req: Request, auth: { userId: string }): Promise<Response> {
...
export async function handleUpdateProfile(req: Request, auth: { userId: string }, id: number): Promise<Response> {
...
export async function handleDeleteProfile(auth: { userId: string }, id: number): Promise<Response> {
  const deleted = await deleteProfile(id, auth.userId);
  if (!deleted) return error("Profile not found", 404);
  return json({ ok: true });
}

export async function handleReorderProfiles(req: Request, auth: { userId: string }): Promise<Response> {
...
export async function handleCheckHandle(url: URL, auth: { userId: string }): Promise<Response> {
...
export async function handleExport(auth: { userId: string }, id: number): Promise<Response> {
  const data = await exportProfileData(id, auth.userId);
  if (!data) return error("Profile not found", 404);
  return json(data);
}

export async function handleTimeline(auth: { userId: string }, id: number): Promise<Response> {
  const result = await getTimelineForProfile(id, auth.userId);
  if (!result) return error("Profile not found", 404);
  return json(result);
}

export async function handleSnapshot(url: URL, auth: { userId: string }, id: number): Promise<Response> {
...
export async function handleTrends(url: URL, auth: { userId: string }, id: number): Promise<Response> {
  const opts = {
...
export async function handleDaysSince(auth: { userId: string }, id: number): Promise<Response> {
  const result = await getDaysSinceForProfile(id, auth.userId);
  if (!result) return error("Profile not found", 404);
  return json(result);
}

export async function handleCompare(url: URL, auth: { userId: string }, id: number): Promise<Response> {
...
export async function handleCorrelations(auth: { userId: string }, id: number): Promise<Response> {
