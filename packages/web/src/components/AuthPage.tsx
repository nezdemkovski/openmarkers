        ...
        <div className="max-w-5xl mx-auto px-5 h-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            {ready && (isAuthenticated ? (
              <Button size="sm" onClick={goToApp} className="ml-1.5 gap-1.5 rounded-full h-7 px-3 text-xs">
                {t("goToApp")}
                <ArrowRight className="w-3 h-3" />
              </Button>
            ) : (
              <Button size="sm" onClick={() => openAuth("login")} className="ml-1.5 gap-1.5 rounded-full h-7 px-3 text-xs">
                {t("authLogin")}
                <ArrowRight className="w-3 h-3" />
              </Button>
            ))}
      ...
      <main className="flex-1 flex items-center justify-center px-5 py-10 sm:py-16">
        <div className="w-full max-w-5xl">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-start">
            ...
            <div className="lg:sticky lg:top-20">
              ...
              {ready && (
                <div className="mt-6 flex items-center gap-3">
                  {!isAuthenticated && (
                    <Button onClick={() => openAuth("signup")} className="rounded-full gap-2 text-xs h-8 px-5">
                      {t("getStarted")}
                      <ArrowRight className="w-3 h-3" />
                    </Button>
                  )}
            ...
            {!ready ? (
              <div className="rounded-2xl border border-border/40 bg-card flex items-center justify-center min-h-[280px]">
                <Loader2 className="w-6 h-6 text-muted-foreground/40 animate-spin" />
              </div>
            ) : (
              <div className="rounded-2xl border border-border/40 bg-card overflow-hidden flex flex-col max-h-[min(70vh,560px)]">
