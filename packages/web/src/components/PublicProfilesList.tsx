    ...
    <div className="min-h-screen bg-background flex flex-col">
      <header className="sticky top-0 bg-background/95 backdrop-blur border-b px-4 py-3 flex items-center justify-between z-30">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-muted-foreground">{t("openProfiles")}</span>
        </div>
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-5 py-8">
        <div className="flex items-center gap-2.5 mb-1">
          ...
          <h1 className="text-lg font-semibold text-foreground">{t("openProfiles")}</h1>
        </div>
        <p className="text-sm text-muted-foreground/60 mb-6">{t("openProfilesDesc")}</p>
        ...
        {profiles === null ? (
          ...
          <div className="rounded-xl border border-border/40 bg-card overflow-hidden">
            <table className="w-full">
              <tbody>
                {profiles.map((p, i) => (
                  <tr key={p.handle} className="group border-b border-border/20 last:border-b-0">
