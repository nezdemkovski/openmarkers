...
export function makeI18n(lang: Lang): I18n {
  return {
    ...
    tCat: (catId, field) =>
      translations[lang]?.categories?.[catId]?.[field] || translations.en.categories[catId]?.[field] || "",
    tBio: (bioId, field) =>
      translations[lang]?.biomarkers?.[bioId]?.[field] || translations.en.biomarkers[bioId]?.[field] || "",
  };
}
