...
function fmtVal(v: number | string | null | undefined): string {
  if (v == null) return "\u2014";
  if (typeof v === "number")
    return Number.isInteger(v) ? String(v) : (+v.toPrecision(4)).toString();
  return String(v);
}
...
export function buildPrompt(
  ...
  lines.push(
    "You are a medical data analyst. Analyze the following lab results for a patient.",
  );
  ...
  lines.push(
    "- Correlations between related biomarkers (e.g. iron panel, lipid panel)",
  );
  ...
  if (outOfRange.length > 0) {
    ...
    for (const { bio, latest, trend } of outOfRange) {
      const ref = [
        bio.refMin != null ? `min: ${bio.refMin}` : null,
        bio.refMax != null ? `max: ${bio.refMax}` : null,
      ]
        .filter(Boolean)
        .join(", ");
      let trendStr = "";
  ...
  if (warnings.length > 0) {
    lines.push(
      "## \u26a1 Approaching Limits (in range but trending toward boundary)",
    );
  ...
  for (const cat of categories) {
    ...
    for (const bio of cat.biomarkers) {
      ...
      const qualResults = bio.results.filter(
        (r) => typeof r.value === "string",
      );
      const ref = [
        bio.refMin != null ? bio.refMin : null,
        bio.refMax != null ? bio.refMax : null,
      ].filter((v): v is number => v !== null);
      const refStr = ref.length > 0 ? ` [ref: ${ref.join("\u2013")}]` : "";

      if (numResults.length > 0) {
        const history = numResults
          .map((r) => `${r.date}: ${fmtVal(r.value)}`)
          .join(", ");
  ...
  if (user.dateOfBirth) {
    ...
    if (bioAgeResults.length > 0) {
      ...
      lines.push(
        "Calculated using Levine's PhenoAge formula (2018) from 9 blood biomarkers.",
      );
      lines.push("");
      ...
      for (const s of latest.scores) {
        lines.push(
          `  - ${s.id}: ${s.value} ${s.unit} → score ${s.score > 0 ? "+" : ""}${s.score.toFixed(2)}`,
        );
      }
  ...
  if (correlations.length > 0) {
    ...
    for (const group of correlations) {
      const names = group.matched
        .map((id) => tBio(id, "name") || id)
        .join(", ");
  ...
  lines.push(
    "2. **Key Concerns** \u2014 Out-of-range values ranked by clinical significance, with possible causes",
  );
  lines.push(
    "3. **Trends** \u2014 Notable changes over time (improving or worsening patterns)",
  );
  lines.push(
    "4. **Correlations** \u2014 Related biomarkers that together suggest a specific condition",
  );
  ...
  lines.push(
    "7. **Suggested Follow-up** \u2014 What to discuss with a doctor, additional tests to consider",
  );
  ...
  if (lang && lang !== "en") {
    ...
    lines.push("");
    lines.push(
      `IMPORTANT: Please write your entire response in ${langEntry?.name || lang}.`,
    );
  }
