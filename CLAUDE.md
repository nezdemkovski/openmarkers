...
OpenMarkers is an open-source biomarker tracker with a Bun backend, Postgres database (via Drizzle ORM + `bun:sql`), Better Auth for authentication, and an MCP server for AI agent access. The React SPA visualizes blood test / lab results over time using Recharts, fetching data from an authenticated API.
